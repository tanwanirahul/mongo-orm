...
class MongoResource(GenericResource):
    ...
    def base_urls(self):
        ...
        return [
            ...
            url(r"^(?P<resource_name>%s)/set/(?P<%s_list>\w[\w/;-]*)%s$" % (self._meta.resource_name,
                self._meta.detail_uri_name, trailing_slash(
                )), self.wrap_view(
                'get_multiple'), name="api_get_multiple"),
            url(r"^(?P<resource_name>%s)/(?P<%s>\w[\w-]*)%s$" % (self._meta.resource_name, self._meta.detail_uri_name,
                trailing_slash(
                )), self.wrap_view(
