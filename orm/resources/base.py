...
class GenericResource(Resource):
    ...
    def wrap_view(self, view):
        ...
        def wrapper(request, *args, **kwargs):
            try:
                ...
                logger.error(
                    "%s failed with bad request error %s" % (request.path, e),
                    exc_info=False)
                data = {"error": e.args[0] if getattr(e, 'args') else ''}
                return self.error_response(request, data, response_class=http.HttpBadRequest)
            except ValidationError as e:
                logger.error(
                    "%s failed with validation error %s" % (request.path, e),
                    exc_info=False)
                data = {"error": e.message}
                return self.error_response(request, data, response_class=http.HttpBadRequest)
            except Exception as e:
                logger.error(
                    "%s failed with unknown error %s" % (request.path, e),
                    exc_info=True)
    ...
    def remove_api_resource_names(self, url_dict):
        ...
        # We are overriding this method so that it does not delete the resource
        # name
        return url_dict
    ...
    def post_list(self, request, **kwargs):
        ...
        if self._meta.__dict__.get('businessLogicLayer', None):
            self._meta.__dict__['businessLogicLayer'].process_bll(
                data=updated_bundle.data)
    ...
    def get_list(self, request, **kwargs):
        ...
        paginator = self._meta.paginator_class(request_data=request.GET, objects=sorted_objects, count=count, resource_uri=self.get_resource_uri(
    ...
    def obj_get_list(self, bundle, **kwargs):
        ...
        try:
            ...
            if filters.get('fields'):
                ...
                required_fields = []
            stats = self.get_object_list(
                bundle.request, applicable_filters, required_fields, **db_params)
            auth_objs = self.authorized_read_list(
                stats.get("objects", []), bundle)
    ...
    def get_pagination_parameters(self, request):
        paginator = self._meta.paginator_class(request.GET, [], 20, resource_uri=self.get_resource_uri(
        ), limit=self._meta.limit, max_limit=self._meta.max_limit, collection_name=self._meta.collection_name)
    ...
    def obj_get(self, bundle, **kwargs):
        try:
            pk = bundle.request.user.get(
                "id", "") if kwargs["pk"].lower() == "me" else kwargs["pk"]
...
#            self.authorized_create_detail(self.get_object_list(bundle.request), bundle)
            ...
            raise BadRequest(
                "Cannot create a new instance for this resource ", e.message)
        return bundle
    ...
    def obj_update(self, bundle, skip_errors=False, request=None, **kwargs):
        ...
        try:
            data = bundle.data
            update_results = self.update_obj(
                None, kwargs["pk"], data, safe=True, fsync=True)
