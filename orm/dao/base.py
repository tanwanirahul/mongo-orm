...
class BaseDAO(object):
    ...
    def _get_spec(self, doc_id):
        if not type(doc_id) in (unicode, str, ObjectId):
            raise TypeError(
                "Id must be an instance of unicode, str, or ObjectId not %s" % (type(doc_id)))
    ...
    def _get_orderby(self, fields):
        ...
        def build_orderby(elem):
            if type(elem) in (tuple,):
                ...
                if not order_type:
                    raise ValueError(
                        "valid possible values for order type are : %s" %
                        ORDER_TYPE_MAPP.keys())
...
class MongoDAO(BaseDAO):
    ...
    # Injecting the datasource for this object. This may simply be a mongoDB
    # collection.
    def __init__(self, datasource, auditlogger=None):
    ...
    def update_fields(self, spec=None, document={}, **kwargs):
        ...
        resp = self.datasource.update(
            {"_id": ObjectId(_id)}, {"$set": document}, **kwargs)
    ...
    def remove_by_id(self, doc_id, **kwargs):
        ...
        spec.update({"isDeleted": {"$ne": True}})
        resp = self.datasource.update(
            spec, {"$set": {"isDeleted": True}}, **kwargs)
        if not resp.get("updatedExisting", False):
            raise DoesNotExist("The object id specified does not exist")
        return resp
    ...
    def aggregate(self, query, field):
        ...
        '''
        result = self.datasource.aggregate(
            [{"$match": query}, {"$group": {"_id": field, "count": {"$sum": 1}}}])
        return result

    def _post_process_get_filtered(self, objects):
        objects_and_count = {"count":
                             objects.count(), "objects": list(objects)}
...
class CollectionDoc(MongoDAO):
    ...
    def _get(self, query, projection, **kwargs):
        ...
        if projection:
            cursor = self.datasource.find(
                query, projection, **kwargs).sort([("_id", -1)]).limit(1)
        else:
            cursor = self.datasource.find(
                query, **kwargs).sort([("_id", -1)]).limit(1)
        if cursor.count() > 0:
            return cursor.next()
    ...
    def get_filtered(self, query={}, fields=[], orderby={}, **kwargs):
        ...
        spec.update({"isDeleted": {"$ne": True}})
        if projection:
            objects = self.datasource.find(
                spec, projection, **kwargs).sort(orderby)
        else:
            objects = self.datasource.find(spec, **kwargs).sort(orderby)
        return self._post_process_get_filtered(objects)
...
class ListElement(MongoDAO):
    # Injecting the data source for this object. This may simply be a mongoDB
    # collection. + Audit logger + sub-resource field name.
    ...
    def _cursor_to_list_of_objects(self, docs):
        results = []
        for obj in docs:
            dataset = obj[self.field] if type(obj[self.field]) in (
                list,) else obj[self.field].values()
            for res in dataset:
                if res is None:
                    continue
...
class GridDAO(BaseDAO):
    # Injecting the datasource(fs) for this object. This may simply be a
    # object fetcher.
