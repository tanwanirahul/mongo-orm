    ...
    def __init__(self, skip_validation=False, required_fields=[], is_list_type=False, wrapped_in=None):
        ...
        if not type(required_fields) in (list,):
            raise ImproperlyConfigured(
                "Required fields attribute of Meta must be a list.")
        if is_list_type and wrapped_in is None:
            raise ImproperlyConfigured(
                "If is_list_type is set to True, wrapped_in must be specified.")
...
class BaseModel(object):
    ...
    def _python_to_jsonschema(cls, structure):
        if type(structure) in (dict,):
            ...
            json_struct["properties"] = dict((name, BaseModel._python_to_jsonschema(value))
    ...
    def __init__(self, dao=None, activity_logger=None):
        ...
        if not hasattr(self, "structure"):
            raise ImproperlyConfigured(
                "Models schema must be specified. Define structure attribute in Model class for the same.")
        model_properties = {}
...
#        self.activity_logger.viewed(client,data)
    ...
    def get_filtered(self, client, filters={}, fields=[], **kwargs):
        if not self.properties["is_list_type"]:
            ...
            data = self.dao.get_filtered(
                query=filters, fields=fields, **kwargs)
        else:
    ...
    def validate(self, data):
        ...
        try:
            ...
            raise exceptions.ValidationError(
                "The data specified is not adhering to the required schema. %s" % e.message)
    ...
    def _pre_process_special_cases(self, value):
        if value == "True" or value == "true" or value == True:
            ...
            return False
        # Support for in and notin kind filters.
    ...
    def type_conversion(self, field_type, value):
        try:
            # For boolean types, values are already pre_processed. Hence, no
            # conversion.
            if field_type in (bool,) or isinstance(value, bool):
                return value
            # Support for in and notin kind filters.
    ...
    def __assert_type(self, data, data_type, attribute_name):
        if type(data) not in (data_type,) and not issubclass(type(data), data_type):
            raise ValidationError(
                "Schema validation failed for field %s." % attribute_name)
    ...
    def __str_to_datetime(self, data, attribute_name=None):
        ...
        try:
            ...
            raise ValidationError(
                "Datetime specified is not in ISO Format for field %s." % attribute_name)
